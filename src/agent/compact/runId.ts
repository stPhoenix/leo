...
export interface GenerateCompactRunIdOptions {
  readonly now?: () => Date;
  readonly tail?: () => string;
}

export function generateCompactRunId(opts: GenerateCompactRunIdOptions = {}): string {
  const d = (opts.now ?? ((): Date => new Date()))();
  const tail = (opts.tail ?? ((): string => randomTail(6)))();
  return `cmp-${formatTimestamp(d)}-${tail}`;
}
